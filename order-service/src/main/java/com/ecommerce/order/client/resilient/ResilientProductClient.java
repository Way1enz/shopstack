...
// Instance name "product-service" matches application.yml. decrementStock/restock take a
// caller-supplied idempotencyKey rather than generating one internally: @Retry re-invokes
// this method from scratch on every attempt, so a key generated inside it would change on
// every retry. The caller (OrderService) fixes the key once before the first attempt.
@Component
